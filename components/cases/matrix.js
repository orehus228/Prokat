...
import { getLinks, addLink, removeLink, getLinksForSource } from '../../services/order-data.js';
...
let currentMatrixSourcePath = null; // для открытия с выделением

const MATRIX_PRESETS_KEY = STORAGE_KEYS.MATRIX_PRESETS;
const MATRIX_FULLNAMES_KEY = STORAGE_KEYS.MATRIX_FULLNAMES;
...
function getAllItemPaths() {
  ...
  function traverse(obj, path) {
    if (Array.isArray(obj)) {
      obj.forEach(item => {
        const full = path.length ? path.join('|') + '|' + item : item;
        res.push(full);
      });
    ...
    filtered.forEach((source, idx) => {
      ...
      allTargets.forEach(target => {
        if (source.full === target.full) {
          ...
          const srcLinks = links[source.full] || [];
          const link = srcLinks.find(l => l.target === target.full);
          const totalMultiplier = link ? link.multiplier : 0;
  ...
  for (let src in preset.links) {
    ...
    preset.links[src].forEach(pl => {
      ...
      if (existing) {
        ...
        state.links[src].push({ target: pl.target, multiplier: pl.multiplier });
      }
    });
  }
  saveState();
...
window.editMatrixCell = async function(td, src, target) {
  ...
  const val = await showPrompt(
    ...
    currentVal !== '' ? String(currentVal) : '1'
  );
  if (val === null) return;
  ...
  if (isNaN(num) || num === 0) {
    ...
    const state = getState();
    if (!state.links[src]) state.links[src] = [];
    const existingLink = state.links[src].find(l => l.target === target);
    if (existingLink) existingLink.multiplier = num;
    else state.links[src].push({ target, multiplier: num });
  }
  saveState();
  renderMatrix();
  updateLinkCountOrder();
