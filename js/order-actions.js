// order-actions.js — Обработчики событий страницы заказа (с проверкой доступности)
import { editorData, getItemProps, getCommonCases, getStockValue, getAvailableQuantity } from './data.js';
...
import {
    ...
    getSelectedOption, orderProject
} from './order.js';
import {
    getStockValue as getStockValueHelper,
    setValueOrder, getActiveItemsOrder, updateChildRowsForPath, buildInfoHtml,
...
// ============================================================
// НОВАЯ ФУНКЦИЯ ПРОВЕРКИ ДОСТУПНОСТИ (вызывается из обработчиков)
// ============================================================
function checkAndWarnAvailability(path, requestedQty) {
    // Проверяем, есть ли даты проекта
    if (!orderProject.start_date || !orderProject.end_date) return true; // нет дат – не проверяем

    const result = getAvailableQuantity(path, orderProject.start_date, orderProject.end_date, requestedQty, orderProject.id);
...
// ============================================================
// ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (исправлены с вызовом проверки)
// ============================================================
function handleQtyChange(path, delta) {
    ...
    const sq = getStockValueHelper(path);
    if (val > sq) {
    ...
    if (!availableCheck) {
        // Если конфликт – возвращаем предыдущее значение
...
function handleMultiPieceChange(path, idx, delta) {
    ...
    // Проверка доступности в проектах (суммируем все варианты)
...
function handleMultiCaseChange(path, idx, delta) {
    ...
    if (opt && opt.qty > 0) {
        ...
        const availableCheck = checkAndWarnAvailability(path, totalVal);
        if (!availableCheck) {
            const oldVal = parseInt(inputCases.dataset.oldValue) || 0;
            inputCases.value = oldVal;
            return;
        }
        inputCases.dataset.oldValue = val;
        const inputPieces = childRow.querySelector(`.child-multi-pieces[data-idx="${idx}"]`);
...
function handleCommonQtyChange(path, caseId, delta) {
    ...
    if (p) {
        ...
        // Проверка доступности в проектах (суммируем все упаковки)
...
// ============================================================
// ОСНОВНОЙ ОБРАБОТЧИК КЛИКОВ (без изменений, только удалён обработчик remove-common-pack)
// ============================================================
function handleContainerClick(e) {
    ...
    // Удалён обработчик для .remove-common-pack
