...
/**
 * Менеджер грузовиков.
 * Позволяет создавать, редактировать, удалять пресеты грузовиков.
 * @module ui/components/TruckManager
 */
...
import { getTruckPresets, getTruckPresetById, createTruckPreset, updateTruckPreset, deleteTruckPreset } from '../../services/trucks.js';
import { getSelectedTruckIds, setSelectedTruckIds } from '../../services/trucks.js';
...
export class TruckManager {
  ...
  _editTruck(id) {
    ...
    if (!t) {
      showToast('Грузовик не найден', 'error');
      return;
    }
  ...
  async _deleteTruck(id) {
    ...
    try {
      ...
      // Если грузовик был выбран — удаляем из выбранных
      const selected = getSelectedTruckIds();
      const filtered = selected.filter(sid => sid !== id);
      setSelectedTruckIds(filtered);
      this._render();
  ...
  _saveTruck() {
    ...
    if (!name) {
      showToast('Введите название грузовика', 'warning');
      return;
    }
    if (isNaN(length) || length <= 0) {
      showToast('Длина должна быть положительным числом', 'warning');
      return;
    }
    if (isNaN(width) || width <= 0) {
      showToast('Ширина должна быть положительным числом', 'warning');
      return;
    }
    if (isNaN(height) || height <= 0) {
      showToast('Высота должна быть положительным числом', 'warning');
      return;
    }
...
export function openTruckManager(onClose) {
  if (truckManagerInstance) {
    truckManagerInstance.close();
  }
