...
// Исправлена ошибка в методе close (обращение к modal.querySelector до проверки)
// и добавлена проверка на существование элементов.
...
export class CommonCasesManager {
  ...
  _bindEvents() {
    ...
    modal.addEventListener('click', (e) => {
      ...
      if (action === 'edit') this._editCase(id);
      else if (action === 'delete') this._deleteCase(id);
    });
  }
  ...
  _editCase(id) {
    ...
    if (!c) { showToast('Кофр не найден', 'error'); return; }
    const modal = this._modalEl;
    if (!modal) return;
  ...
  async _deleteCase(id) {
    const c = getCommonCaseById(id);
    if (!c) { showToast('Кофр не найден', 'error'); return; }
    const confirmed = await showConfirm(`Удалить кофр "${c.name}"?`);
    if (!confirmed) return;
    try {
      deleteCommonCase(id);
      this._render();
  ...
  close() {
    if (this._modalEl) {
      ...
      // Безопасно очищаем форму, проверяя наличие элементов
      const modal = this._modalEl;
      const fields = ['newCaseName', 'newCaseQty', 'newCaseDim', 'newCaseWeight', 'newCaseMaxWeight', 'newCaseMaxVolume'];
...
export function openCasesManagerModal(onClose) {
  if (commonManagerInstance) commonManagerInstance.close();
