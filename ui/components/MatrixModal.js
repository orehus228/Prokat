...
/**
 * Модалка матрицы привязок (линков).
 * Отображает таблицу, где строки — источники, столбцы — цели,
 * позволяет редактировать множители, сохранять/загружать пресеты,
 * фильтровать и масштабировать.
 * @module ui/components/MatrixModal
 */
...
export class MatrixModal {
   ...
   * @param {string} [options.sourcePath] - путь для выделения
   * @param {boolean} [options.showPresets=true] - показывать панель пресетов
   * @param {string} [options.category] - категория для открытия
  ...
  /**
   * Инициализирует модалку (создаёт DOM, если его нет).
   */
  init() {
  ...
  /**
   * Создаёт DOM-структуру модалки, если её нет.
   */
  _createModalDOM() {
  ...
  _bindEvents() {
    ...
    // Управление отображением полных названий (добавляем чекбокс)
    ...
    // Подписка на изменение привязок
  ...
  // ============================================================
  // ЗАГРУЗКА СОСТОЯНИЯ ПОЛНЫХ ИМЁН
  // ============================================================
